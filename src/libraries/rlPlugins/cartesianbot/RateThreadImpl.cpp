...
void CartesianBot::run() {
    if (cmc_status>0) {  // If it is movement
        ...
        if (done) {
            ...
            lawxP.resize(3);
            lawxP = (eP * GAIN * (msCartesian/1000.0)) + xPdotd;  // GAIN=0 => lawxP = xPdotd;
...
//            yarp::sig::Matrix Ja_pinv(pinv(Ja));
            ...
            qdot[0] = trajOz->getdot(sTime) + GAIN*(msCartesian/1000.0)*eoz;  // lawoz
            qdot[1] = toDeg(t[0]);
            qdot[2] = toDeg(t[1]);
            qdot[3] = toDeg(t[2]);
            double eOzPP = trajOzPP->get(sTime) - realDeg[4];
            qdot[4] = trajOzPP->getdot(sTime) + GAIN*(msCartesian/1000.0)*eOzPP;  // lawOzP
