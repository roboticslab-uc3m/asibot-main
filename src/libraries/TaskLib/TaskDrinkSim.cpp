...
bool TaskDrinkSim::run() {
    ...
    //if (!_quiet) printf("[TaskDrinkSim] Got info on redCan: %s\n",bIn.toString().c_str());
    X_0_redCan[2] +=.05; // to avoid collision with table
    if (!_quiet) printf("[TaskDrinkSim] X_0_redCan: %f %f %f.\n",X_0_redCan[0],X_0_redCan[1],X_0_redCan[2]);
    //
    ...
    //
    {
        ...
        printf("[TaskDrinkSim] Commanding Movj to targets: {0, -.3, 0.8, 90, 0}... [wait]\n");
        _cartesianClient.movj(targets);
        _cartesianClient.wait();
    }
    {
        double targets[5] = {-.5, -.5, 0.3, 90, 0};
        printf("[TaskDrinkSim] Commanding Movj to targets: {-.5, -.5, 0.3, 90, 0}... [wait]\n");
        _cartesianClient.movj(targets);
        Time::delay(.1);
        _cartesianClient.wait();
    }
    {
        ...
        printf("[TaskDrinkSim] Commanding Movl to targets: {%f, %f, %f, 90, 0}... [wait]\n",targets[0],targets[1],targets[2]);
    ...
    bOut.addInt(1);
    _worldRpcClient.write(bOut, bIn);
    if (!_quiet) printf("[TaskDrinkSim] Grab redCan: %s\n",bIn.toString().c_str());
    //
    {
        double targets[5] = {-.5, -.5, 0.3, 90, 0};
        printf("[TaskDrinkSim] Commanding Movl to targets: {-.5, -.5, 0.3, 90, 0}... [wait]\n");
        //_cartesianClient.movl(targets);
