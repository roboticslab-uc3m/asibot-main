...
void SegmentorThread::run() {
    ...
    for (int i=0;i<numBlobs;i++) {  // from biggest to smallest
        ...
        if (locate == "corners") {
            CvRect bb = bigBlob.GetBoundingBox();
            Bottle b_c1;
            b_c1.addDouble(myx-bb.width/2.0);
            b_c1.addDouble(myy-bb.height/2.0);
            container.addList() = b_c1;
            Bottle b_c2;
            b_c2.addDouble(myx-bb.width/2.0);
            b_c2.addDouble(myy+bb.height/2.0);
            container.addList() = b_c2;
            Bottle b_c3;
            b_c3.addDouble(myx+bb.width/2.0);
            b_c3.addDouble(myy+bb.height/2.0);
            container.addList() = b_c3;
            Bottle b_c4;
            b_c4.addDouble(myx+bb.width/2.0);
            b_c4.addDouble(myy-bb.height/2.0);
            container.addList() = b_c4;
        } else if (locate == "square") {
            CvRect bb = bigBlob.GetBoundingBox();
            Bottle b_s1;
            b_s1.addDouble(myx-bb.width/2.0);
            b_s1.addDouble(myy-bb.width/2.0);
            container.addList() = b_s1;
            Bottle b_s2;
            b_s2.addDouble(myx-bb.width/2.0);
            b_s2.addDouble(myy+bb.width/2.0);
            container.addList() = b_s2;
            Bottle b_s3;
            b_s3.addDouble(myx+bb.width/2.0);
            b_s3.addDouble(myy+bb.width/2.0);
            container.addList() = b_s3;
            Bottle b_s4;
            b_s4.addDouble(myx+bb.width/2.0);
            b_s4.addDouble(myy-bb.width/2.0);
