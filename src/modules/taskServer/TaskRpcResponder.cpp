...
bool TaskRpcResponder::read(ConnectionReader& connection) {
    ...
    if ((in.get(0).asString() == "help")||(in.get(0).asVocab() == VOCAB_HELP)) {  // help //
        ...
        printf("[TaskRpcResponder] Tool set to: %d\n", tool);
        Bottle tweek;
...
void TaskRpcResponder::setCartesianInterface(yarp::dev::ICartesianControl* _icart) {
    icart = _icart;
}

/************************************************************************/

void TaskRpcResponder::setPositionInterface(yarp::dev::IPositionControl* _ipos) {
    ipos = _ipos;
}
