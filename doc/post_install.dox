...
/**
 * \page post_install Simulation and Basic Control: Now what can I do?

Now that you have installed the basic ASIBOT repository, you're probably wondering what to do.

@section post_install_init_comms Initializing the communication server
...
@section post_install_launch_simulator Launching the simulator
...
@section post_install_changing_parameters Changing parameters
...
Each layer of ASIBOT modules has parameters that can be modified three different ways. They are parsed in the following order (the second parsing overwrites the first, and the third one overwrites both):
  ...
  \li Configuration files (*.ini or *.xml extension). These files are downloaded from asibot-main/share and installed to asibot-main/build/share/asibot with the <i>cmake</i> command we issued. Within this folder, separate folders are maintained for modules and module layers.
...
Let's say, for example, our graphic card supports offscreen rendering and we want \ref cartesianServer to load an enviroment that has cameras in the simulated environment. We can execute the program touching the parameters at the command line level which, as we have said, are parsed last:
...
If we want to affect this parameter at a more persistent level, we can change the configuration file. For this specific case, the \ref cartesianServer configuration file is located at <i>$ASIBOT_ROOT/share/cartesianServer/conf/cartesianServer.ini</i>. In this file, we can see that most parameters are commented out (the <b>//</b> characters). This is a common convention to indicate these are the default parameters set in the headers (the first parsed, as explained previously). Here, we would search to subsitute the line:
...
@section post_install_interfacing Interfacing

We can interact with this module through port commands as described in
<a class="el" href="group__testRaveBot.html#testRaveBot_interfacing">testRaveBot</a>
and <a class="el" href="group__cartesianServer.html#cartesianServer_interfacing">cartesianServer</a>,
or through the different language APIs as can be seen in the different
<a class="el" href="group__asibot__examples.html">ASIBOT examples</a>.

@section post_install_even_more Even more!

Done! You are now probably interested in one of the following links:

  \li \ref install_vision_on_ubuntu
  \li You can now compile and install any of the <a class="el" href="modules.html">other modules</a>.
  \li These modules are actually packed up and ready to go in our \ref asibot_applications!

<hr>

This page can be edited at asibot-main/doc/post_install.dox.
