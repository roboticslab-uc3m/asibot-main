 ...
 * @ingroup PrintBot
 *
 * PrintBot creates a virtual (text-mode) N-DOF robot and exposes a YARP_dev controlboard
...
class PrintBot : public DeviceDriver, public RateThread, public IPositionControl, public IVelocityControl, public IEncoders {
 public:
  ...
  PrintBot() : RateThread(THREAD_RATE) {}  // In ms
 ...
 // ------------------- DeviceDriver Related ------------------------------------
  virtual bool open(Searchable& config) {
    ...
    if(config.check("help")) {
       printf("\n");
       printf("Usage: printbot --help  -------> This help\n");
       printf("Usage: printbot --DOF [DOF]  --> [DOF] degrees of freedom (defaults to %d, max %d)\n\n",DEFAULT_NUM_MOTORS,MAX_NUM_MOTORS);
       exit(1);
    }
